...
if __name__ == '__main__':
    ...
    for t in range(1):
        ...
        torch.save(best_model, 'output/model_paragraph_level_BiLSTM_label_embedding.pt')
    ...
    open('output/paragraph_level_BiLSTM/label_embedding/test_acc/' + str(np.mean(np.array(test_acc_list))) + '.txt', 'w')
    open('output/paragraph_level_BiLSTM/label_embedding/test_f1/' + str(np.mean(np.array(test_f1_list))) + '.txt', 'w')
    open('output/paragraph_level_BiLSTM/label_embedding/valid_acc/' + str(np.mean(np.array(valid_best_acc_list))) + '.txt', 'w')
    open('output/paragraph_level_BiLSTM/label_embedding/valid_f1/' + str(np.mean(np.array(valid_best_f1_list))) + '.txt', 'w')
