    ...
    def forward(self, sentences_list, label_list):
        ...
        for sentence in sentences_list:
            ...
            if torch.cuda.memory_allocated(0) > 10000000000:
                label_list = label_list[:len(sentence_embeddings_list)]
                break
        ...
        outputs = self.bert_model_2(inputs_embeds=sentence_embeddings_list, labels=label_list)

        # print("after bert2: ", torch.cuda.memory_allocated(0))

        loss = outputs.loss
        logits = outputs.logits  # 1 * sentence_num * 7
