...
from models.paragraph_level_BiLSTM_label_embedding_sim import MyModel
...
if __name__ == '__main__':
    ...
    for t in range(1):
        ...
        torch.save(best_model, 'output/model_paragraph_level_BiLSTM_label_embedding_sim.pt')
    ...
    open('output/paragraph_level_BiLSTM/label_embedding_sim/test_acc/' + str(np.mean(np.array(test_acc_list))) + '.txt', 'w')
    open('output/paragraph_level_BiLSTM/label_embedding_sim/test_f1/' + str(np.mean(np.array(test_f1_list))) + '.txt', 'w')
    open('output/paragraph_level_BiLSTM/label_embedding_sim/valid_acc/' + str(np.mean(np.array(valid_best_acc_list))) + '.txt', 'w')
    open('output/paragraph_level_BiLSTM/label_embedding_sim/valid_f1/' + str(np.mean(np.array(valid_best_f1_list))) + '.txt', 'w')
