...
from models.paragraph_level_BiLSTM_label_embedding_MLP_pre_large import MyModel
...
if __name__ == '__main__':
    ...
    for t in range(1):
        ...
        train_batch_list = get_train_batch_list_large(train_data_list, BATCH_SIZE, each_data_len=0)
        ...
        torch.save(best_model, 'output/model_paragraph_level_BiLSTM_label_embedding_MLP_pre_large_' + str(cheat) + '_' + str(mask_p) + '.pt')
    ...
    open('output/paragraph_level_BiLSTM/label_embedding_MLP_pre_large_' + str(cheat) + '_' + str(mask_p) + '/test_acc/' + str(np.mean(np.array(test_acc_list))) + '.txt', 'w')
    open('output/paragraph_level_BiLSTM/label_embedding_MLP_pre_large_' + str(cheat) + '_' + str(mask_p) + '/test_f1/' + str(np.mean(np.array(test_f1_list))) + '.txt', 'w')
    open('output/paragraph_level_BiLSTM/label_embedding_MLP_pre_large_' + str(cheat) + '_' + str(mask_p) + '/valid_acc/' + str(np.mean(np.array(valid_best_acc_list))) + '.txt', 'w')
    open('output/paragraph_level_BiLSTM/label_embedding_MLP_pre_large_' + str(cheat) + '_' + str(mask_p) + '/valid_f1/' + str(np.mean(np.array(valid_best_f1_list))) + '.txt', 'w')
