...
from models.paragraph_level_BiLSTM_one_hot import MyModel
...
if __name__ == '__main__':
    ...
    for t in range(1):
        ...
        torch.save(best_model, 'output/model_paragraph_level_BiLSTM_one_hot.pt')
    ...
    open('output/paragraph_level_BiLSTM/one_hot/test_acc/' + str(np.mean(np.array(test_acc_list))) + '.txt', 'w')
    open('output/paragraph_level_BiLSTM/one_hot/test_f1/' + str(np.mean(np.array(test_f1_list))) + '.txt', 'w')
    open('output/paragraph_level_BiLSTM/one_hot/valid_acc/' + str(np.mean(np.array(valid_best_acc_list))) + '.txt', 'w')
    open('output/paragraph_level_BiLSTM/one_hot/valid_f1/' + str(np.mean(np.array(valid_best_f1_list))) + '.txt', 'w')
